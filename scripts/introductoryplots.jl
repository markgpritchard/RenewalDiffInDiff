    ...
    fig = with_theme(theme_latexfonts()) do
        ...
        let 
            ...
            let 
                ...
                lines!(
                    axs[1], df1.timestamp[10:end], logsol1cf; 
                    color=COLOURVECTOR[3], linestyle=:dash, linewidth=1,
                )
                lines!(
                    axs[1], df1.timestamp, log.(df1.value2); 
                    color=COLOURVECTOR[1], linewidth=1,
                )
                ...
                lines!(
                    axs[2], df1.timestamp[10:end], logincidencecf; 
                    color=COLOURVECTOR[3], linestyle=:dash, linewidth=1,
                )
                lines!(
                    axs[2], df1.timestamp[2:end], log.(incidence1[2:end]); 
                    color=COLOURVECTOR[1], linewidth=1,
                )
                lines!(
                    axs[2], df2.timestamp[2:end], log.(incidence2[2:end]); 
                    color=:black, linewidth=1,
                )
                arrows!(
                    axs[2], 
                    df1.timestamp[10:10], 
                    log.(incidence1[10:10]) .+ 1.2, [ 0 ], 
                    [ -0.6 ]
                )

                band!(
                    axs[3], 
                    df1.timestamp[10:end], 
                    log.(df1.value4[10:end]), 
                    cumulativelogsol1cf; 
                    color=bandcolour
                )
                lines!(
                    axs[3], df1.timestamp[10:end], cumulativelogsol1cf; 
                    color=COLOURVECTOR[3], linestyle=:dash, linewidth=1,
                )
                lines!(
                    axs[3], df1.timestamp[2:end], log.(df1.value4[2:end]); 
                    color=COLOURVECTOR[1], linewidth=1,
                )
                lines!(
                    axs[3], df2.timestamp[2:end], log.(df2.value4[2:end]); 
                    color=:black, linewidth=1,
                )
                arrows!(
                    axs[3], 
                    df1.timestamp[10:10], 
                    log.(df1.value4[10:10]) .+ 1.125, 
                    [ 0 ], 
                    [ -0.6 ]
                )
