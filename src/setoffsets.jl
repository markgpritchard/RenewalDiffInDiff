...
function addoffsetstointerventionarray(originalinterventions::InterventionMatrix{T}; offset=-35:7:35) where T 
    return InterventionArray(originalinterventions; offset)
end

function addoffsetstointerventionarray(originalinterventions::InterventionArray{T}; offset=-35:7:35) where T 
    ...
    for k in axes(_interventions, 3)
        k == 1 && continue 
        interventions = cat(
            interventions, 
